...
export default function ZoneDiagram({ actions }: { actions: Action[] }) {
  ...
  useEffect(() => {
    const updatedData = initialData.map((zoneData) => ({ ...zoneData }));

    actions.forEach((action) => {
      if (action.zone !== 0) {
        updatedData[action.zone - 1].total++;
        if (action.shotResult === "entry") {
          updatedData[action.zone - 1].successful++;
        }
      }
    });

    updatedData.forEach((zoneData) => {
      if (zoneData.total > 0) {
        zoneData.rate = parseFloat(
          ((zoneData.successful / zoneData.total) * 100).toFixed(2)
        );
      }
    });
  ...
  return (
    <div className={classes.container}>
      <h3 className="title">Статистики по зонам</h3>
      <div className="porcentages__container">
        {diagramData.map((zone) => {
          return (
            <>
              <div key={zone.zone} className={`porcentage area_${zone.zone}`}>
                <p className="data">
                  {zone.successful}/{zone.total}
                </p>
                <p className="total">{zone.rate}%</p>
              </div>
            </>
          );
        })}
